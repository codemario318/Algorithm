...
def solution(n, stations, w):
    ...
    pos = 1
    cover = 2*w+1
    print('cover size:',cover)
    for s in stations:
        print('Cur pos:',pos,'station:',s)
        if pos < s-w:
            print('Not cover, size is',s-w-pos)
            answer += ceil((s-w-pos)/cover)
        pos = s+w
        print('End Pos:',pos)

    if pos < n:
        print('not fin, pos:', pos, 'size:',n-pos)
        answer += ceil((n-pos)/cover)

    return answer

from math import ceil
def solution(n, stations, w):
    answer = [stations[i]-stations[i-1]-2*w-1 for i in range(1,len(stations))]
    answer.append(stations[0]-w-1)
    answer.append(n-stations[-1]-w)
    return sum(map(lambda x: ceil(x/(2*w+1)) if x > 0 else 0, answer))
