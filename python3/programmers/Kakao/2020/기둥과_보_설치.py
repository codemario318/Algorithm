...
def solution(n, build_frame):
    def check_bo(x,y):
        if (x,y-1,0) in built or (x+1,y,0) in built:
            return True
        elif (x,y-1,1) built and (x,y+1,1) in built:
            return True
        return False

    def check_col(x,y):
        if x == 0 or (x-1,y,0) in built:
            return True
        elif (x,y-1,1) in built or (x,y,1) in built:
            return True
        return False
    ...
    def build_col(x,y):
        if x == 0 or (x-1,y) in col or

    bo = set()
    col = set()
    for x,y,k,b in build_frame:
        # 보 일때
        if k  :
