...
def soulution(lines):
    logs = []
    for line in lines:
        date,time,runtime = line.split()

        fin = datetime.datetime.strptime(date+time, "%Y-%m-%d%H:%M:%S.%f")
        start = fin - datetime.timedelta(seconds=float(runtime[:-1])+0.001)

        logs.append((int(start.timestamp()*1000),int(fin.timestamp()*1000)))
    logs
    t = list(chain(*logs))
    s,e = min(t),max(t)
