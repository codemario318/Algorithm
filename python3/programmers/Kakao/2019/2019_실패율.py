    ...
    for stage in range(1,N+2):
        ...
        for i in range(len(stages)):
            if stage == stages[i]:
                ...
                break
        try:
            level[stage-1]= stage,total/user_num
        except ZeroDivisionError:
            for i in range(stage-1,N+1):
                level[i] = i+1,0
        user_num -= total
    return list(map(lambda x:x[0],sorted(level[:-1],key=lambda x: (-x[1],x[0]))))
