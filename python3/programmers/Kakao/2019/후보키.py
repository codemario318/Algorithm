...
def solution(relation):
    answer = 0
    keys = [ combinations(range(len(relation[0])),i) for i in range(1,len(relation[0])+1)]
    keys = deque(chain(*keys))

    while keys:
        key = keys.popleft()
        a = len(set(map(lambda x: tuple(x[k] for k in key) ,relation)))

        if a == len(relation):
            answer +=1
            keys = minimality(keys,key)
