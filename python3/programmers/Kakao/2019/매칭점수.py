...
def solution(word, pages):
    answer = 0
    url = re.compile('<meta property="og:url" content="https://a.com"/>')
    bodytag = re.compile('<body> </body>')
    link_rex = re.compile('<a^[\w+]</a>&')
    link_rex.findall('<a href="https://hashcode.co.kr/tos"></a>')
    return answer
...
#
#         words = re.split('[^a-z]+',re.sub('<.+?>','',re.sub('\s+',' ',re.search('<body.*/body>',page,re.I|re.S).group(0))))
#         count = 0
#
#         for text in words:
#             if word == text:
