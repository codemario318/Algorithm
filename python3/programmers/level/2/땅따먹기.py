...
위 예의 경우, 1행의 네번째 칸 (5), 2행의 세번째 칸 (7), 3행의 첫번째 칸 (4)
...
#             for i in range(4):
#                 if before != i:
#                     heappush(hq,(score - land[step+1][i],step+1,i))
#
...
def solution(land):
    ...
    for step in land:
        for i in range(4):
            step[i] += max(map(lambda x: dp[x],[j for j in range(4) if j != i]))
        dp = step
    return max(dp)
