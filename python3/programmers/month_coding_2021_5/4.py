...
def solution(values, edges, queries):
    ...
    parant = [-1 for _ in range(len(values)+1)]
    sub_sum = [-1 for _ in range(len(values)+1)]
    ...
    get_sum_children(tree, values, sub_sum, 1)

    for u, w in queries:
        if w == -1:
            answer.append(sub_sum[u]) 
        else:
            change_p_val(tree, parant, values, sub_sum, u, w)
    return answer

def get_sum_children(tree, values, sub_sum, node):
    total = values[node-1]

    for c in tree[node]:
        total += get_sum_children(tree, values, sub_sum, c)

    sub_sum[node] = total
    return total

def change_p_val(tree, parant, values, sub_sum, u, w):
    p = parant[u]
    while u != 1:
