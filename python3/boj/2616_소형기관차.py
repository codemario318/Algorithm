...
1 초	128 MB	9276	4327	3298	46.900%
문제
...
def solution(passengers, coach_count):
    n = len(passengers)
    prefix_sum = [0]

    for passenger in passengers:
        prefix_sum.append(prefix_sum[-1] + passenger)

    dp = [[0 for _ in range(n + 1)] for _ in range(TRAIN_COUNT)]

    for j in range(coach_count, n + 1):
        dp[0][j] = max(dp[0][j - 1], prefix_sum[j] - prefix_sum[j - coach_count])

    for i in range(1, TRAIN_COUNT):
        for j in range((i + 1) * coach_count, n + 1):
            dp[i][j] = max(dp[i][j - 1], dp[i - 1][j - coach_count] + prefix_sum[j] - prefix_sum[j - coach_count])
...
if __name__ == '__main__':
    _ = int(readline())
    passengers = list(map(int, readline().split()))
    coach_count = int(readline())
    print(solution(passengers, coach_count))
