...
0.3 초 (하단 참고)	512 MB	32239	8536	5704	26.825%
문제
...
class Editor:
    ...
    def add(self, c):
        ...
        new_data = Data(c)
        cur_data = self.cursor
        prev_data = cur_data.prev

        prev_data.next = new_data
        cur_data.prev = new_data

        new_data.prev = prev_data
        new_data.next = cur_data
        self.size += 1
    ...
    def remove(self):
        ...
        if prev_data.val:
            cur_data.prev = prev_data.prev
            prev_data.prev.next = cur_data
            del prev_data
            self.size -= 1

    def cursor_left(self):
        if self.cursor.prev.val:
