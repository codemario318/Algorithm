...
1 초	128 MB	30682	13026	9067	41.212%
...
def search(start, visited):
    visited.add(start)
    if network[start] - visited:
        return max(map(lambda x: search(x,visited),network[start]-visited),key=lambda x:len(x))
    else:
        return visited
