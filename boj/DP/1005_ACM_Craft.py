...
1 초	512 MB	31276	4766	2949	18.216%
문제
...
(동시에 진행이 가능하다) 그리고 4번 건물을 짓기 위해서는 2번과 3번 건물이 모두 건설 완료되어야지만
4번건물의 건설을 시작할수 있다.
...
def dp(n):
    try:
        ...
        try:
            costs[n] = int(delays[n-1]) + max(map(dp,tech_tree[n]))
            return costs[n]
        except KeyError:
            return int(delays[n-1])
