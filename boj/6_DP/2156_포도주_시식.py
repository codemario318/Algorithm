...
2 초	128 MB	35560	12752	9101	34.434%
문제
효주는 포도주 시식회에 갔다.
...
#             return costs[n]
#         except:
#             costs[n] = max(wines[n]+wines[n-1]+dp(n-3), wines[n]+dp(n-2), dp(n-1))
#             return costs[n]
#
...
# wines = [0] + [int(stdin.readline()) for _ in range(n)]
# try:
#     costs = {0:0,1:wines[1],2:wines[1]+wines[2]}
#     print(dp(n))
# except IndexError:
#     print(sum[:3])
...
def dp(n):
    ...
    costs = [0,wines[1],wines[1]+wines[2]] + [0 for _ in range(n-2)]

    for i in range(3,n+1):
        costs[i] = max(wines[i]+wines[i-1]+costs[i-3], wines[i]+costs[i-2], costs[i-1])
    return costs[n]
