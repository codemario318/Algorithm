...
function checkPrivacy(policies, privacy, base) {
    const [date, policy] = privacy.split(SEP);
    const days = dateToDays(date);
    return (base - days) >= policies.get(policy);
}

function dateToDays(date) {
    return toDays(...date.split(DATE_SEP).map(Number));
}
...
function makePolicies(terms) {
    return new Map(terms.map(term => {
        const [type, month] = term.split(SEP);
