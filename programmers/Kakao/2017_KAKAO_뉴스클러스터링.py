...
def solution(str1, str2):
    a_str = [str1.upper()[i:i+2] for i in range(len(str1)-1) if len(re.findall("[^A-Z]",str1.upper()[i:i+2])) == 0]
    b_str = [str2.upper()[i:i+2] for i in range(len(str2)-1) if len(re.findall("[^A-Z]",str2.upper()[i:i+2])) == 0]
    and_num = 0

    for i in range(len(a_str)-1,-1,-1):
        if a_str[i] in b_str:
            and_num += 1
            b_str.pop(b_str.index(a_str[i]))
            a_str.pop(i)

    or_num = len(a_str) + len(b_str) + and_num
